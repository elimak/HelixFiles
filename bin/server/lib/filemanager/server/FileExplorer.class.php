...
class filemanager_server_FileExplorer {
		...
		$listing = sys_FileSystem::readDirectory($dirPath);
		{
			...
			while($_g < $listing->length) {
				...
				if(file_exists($filePath)) {
					...
					if(!is_dir($filePath1)) {
						$fileHelper = new filemanager_server_FileInfoHelper($file, $dirPath, is_dir($filePath1));
		...
		$listing = sys_FileSystem::readDirectory($path);
		{
			...
			while($_g < $listing->length) {
				...
				if(file_exists($dirPath)) {
					$dirPath1 = $path . "/" . $dir;
					if(is_dir($dirPath1)) {
						$fileHelper = new filemanager_server_FileInfoHelper($dir, $path, is_dir($dirPath1));
	...
	public function createTheFolderTree($folderRoot) {
		...
		$pathSplited->pop();
		$rootFolder = new filemanager_server_FileInfoHelper($rootName, $pathSplited->join("/"), true);
	...
	function __toString() { return 'filemanager.server.FileExplorer'; }
}
