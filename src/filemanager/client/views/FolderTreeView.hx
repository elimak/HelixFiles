...
class FolderTreeView extends View
{
	...
	private var _rootData		: FolderVO;		// store the data loaded (full tree in Json/Dynamic)
...
// PRIVATE
	...
	private function buildView() {
		...
		var folderPath	: String = _rootData.path;
		_folderStatus.set(folderPath, _rootFolder.isOpen);

		makeInteractive(_rootFolder, _rootData);
		rootElement.appendChild(_rootFolder.rootElement);
		
		createSubFolders(_rootData, _rootFolder, 1);

		handleOnFolderClick( _rootFolder, _rootData, null);
	}
...
// PRIVATE
	...
	private function handleOnFolderClick( target: FolderUI, folderData: FolderVO, evt: Event ) : Void {
		...
		if( _currentFolderUISelected != null && _currentFolderUISelected != target){
			...
			_fileManager.getListOfFiles(_currentFolderUISelected.path);
		}
